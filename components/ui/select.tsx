  ...
  return (
    <SelectPrimitive.Item
      ...
      <Text className="text-foreground group-active:text-accent-foreground select-none text-sm">
        {typeof children === 'function' ? props.label : children ?? props.label}
      </Text>
    </SelectPrimitive.Item>
  );
}
...
function SelectItemIndicatorIcon({ className }: { className?: string }) {
  return <Icon as={Check} className={cn('text-muted-foreground size-4 shrink-0', className)} />;
}
